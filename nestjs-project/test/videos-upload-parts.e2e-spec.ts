...
describe('videos-upload-parts', () => {
  ...
  let videoProcessingQueue: Queue<VideoProcessingJobData>;
  ...
  beforeAll(async () => {
    ...
    videoProcessingQueue = moduleFixture.get<Queue<VideoProcessingJobData>>(
  ...
  async function createDraftVideo(accessToken: string): Promise<string> {
    ...
    return (res.body as CreateVideoResponseBody).id;
  }
  ...
  it('1.1 rejeita-upload-parts-de-outro-usuario', async () => {
    ...
    expect((res.body as ErrorResponseBody).error).toBe('VIDEO_FORBIDDEN');
  });
  ...
  it('1.2 completa-upload-transiciona-para-processing', async () => {
    ...
    expect((completeRes.body as CompleteResponseBody).status).toBe(
      'processing',
    );
  ...
  it('1.3 rejeita-complete-de-video-ja-processando', async () => {
    ...
    expect((res.body as ErrorResponseBody).error).toBe('INVALID_VIDEO_STATE');
  }, 30000);
});
